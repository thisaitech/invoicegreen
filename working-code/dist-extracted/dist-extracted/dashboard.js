...
async function downloadEstimatePDF(id) {
  ...
  const savedPrevBal = document.getElementById('previous-balance').value;
  ...
  document.getElementById('previous-balance').value = savedPrevBal;
...
async function openEstimatePreview(id) {
  ...
  if (previewModal) {
    ...
    // Generate print HTML for preview - pass items directly to ensure they're used
    const printHTML = window.generatePrintHTML ? window.generatePrintHTML(items) : '';
